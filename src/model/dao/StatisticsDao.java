...
public class StatisticsDao extends Base {
	...
	public RoundSharesVo getRoundShares(Connection conn) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement("SELECT IFNULL(SUM(IF(our_result='Y', IF(difficulty=0, POW(2, ("
					+ GlobalSettings.get("difficulty") + "  - 16)), difficulty), 0)), 0) AS valid,"
					+ "IFNULL(SUM(IF(our_result='N', IF(difficulty=0, POW(2, (" + GlobalSettings.get("difficulty")
					+ " - 16)), difficulty), 0)), 0) AS invalid"
	...
	public ArrayList<BlockVo> getBlocksFoundHeight(Connection conn, int iHeight, int limit) throws SQLException {
		...
		try {
			...
			pstmt = conn.prepareStatement(
					"select  b.*, a.username AS finder, a.is_anonymous AS is_anonymous, ROUND(difficulty * POW(2, 32 - "
							+ coin_base.getTargetBits()
	...
	public PoolStatsVo getPoolStatsHours(Connection conn, int iHours) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement(
					"SELECT IFNULL(COUNT(id), 0) as count, IFNULL(AVG(difficulty), 0) as average, IFNULL(SUM(shares), 0) as shares,"
	...
	public BlockTimeVo getLastBlocksbyTime(Connection conn) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement(
					"SELECT COUNT(id) AS Total, IFNULL(SUM(IF(confirmations > 0, 1, 0)), 0) AS TotalValid,"
	...
	public ArrayList<BlockSolvedAccountVo> getBlocksSolvedbyAccount(Connection conn, int limit) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement(
					"SELECT b.*, a.username AS finder, COUNT(b.id) AS solvedblocks, SUM(b.amount) AS generatedcoins "
	...
	public ArrayList<BlockSolvedWorkerVo> getBlocksSolvedbyWorker(Connection conn, int account_id, int limit)
			throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement(
					"SELECT worker_name AS finder, COUNT(id) AS solvedblocks, SUM(amount) AS generatedcoins"
	...
	public ArrayList<TopContributorsVo> getTopContributors(Connection conn, String type, int limit)
			throws SQLException {
		...
		try {
			...
			case "shares":
				pstmt = conn.prepareStatement(
						"SELECT a.username AS account, a.donate_percent AS donate_percent, a.is_anonymous AS is_anonymous,"
								+ " IFNULL(SUM(IF(s.difficulty=0, POW(2, (" + GlobalSettings.get("difficulty")
								+ " - 16)), s.difficulty)), 0) AS shares"
			...
			case "hashes":
				pstmt = conn.prepareStatement(
						"SELECT a.username AS account, a.donate_percent AS donate_percent, a.is_anonymous AS is_anonymous,"
								+ " IFNULL(SUM(t1.difficulty), 0) AS shares FROM ( SELECT id, IFNULL(IF(difficulty=0, pow(2, ("
								+ GlobalSettings.get("difficulty")
								+ " - 16)), difficulty), 0) AS difficulty, username FROM shares WHERE time > DATE_SUB(now(), INTERVAL 10 MINUTE) AND our_result = 'Y'"
								+ " UNION SELECT share_id, IFNULL(IF(difficulty=0, pow(2, ("
								+ GlobalSettings.get("difficulty") + " - 16)), difficulty), 0) AS difficulty,"
	...
	public ArrayList<BlocksFoundVo> getBlocksFound(Connection conn, int limit) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement(
					"SELECT b.*, a.username AS finder, a.is_anonymous AS is_anonymous, ROUND(difficulty * POW(2, 32 - "
							+ coin_base.getTargetBits() + " ), " + coin_base.getShareDifficultyPrecision()
							+ " ) AS estshares FROM blocks AS b LEFT JOIN accounts AS a ON b.account_id = a.id"
	...
	public double getCurrentHashrate(Connection conn, int interval) throws SQLException {
		...
		try {
			pstmt = conn.prepareStatement("SELECT ( ( SELECT IFNULL(SUM(IF(difficulty=0, POW(2, ( "
					+ GlobalSettings.get("difficulty") + " - 16)), difficulty)), 0) AS"
					+ " shares FROM shares WHERE time > DATE_SUB(now(), INTERVAL ? SECOND) AND our_result = 'Y' ) + ( SELECT IFNULL(SUM(IF(difficulty=0,"
					+ " POW(2, ( " + GlobalSettings.get("difficulty")
					+ " - 16)), difficulty)), 0) AS shares FROM shares_archive WHERE time > DATE_SUB(now(), INTERVAL ? SECOND)"
	...
	/*
	 * public double getExpectedNextDifficulty() { double dDifficulty = 0; double
	 * dNetworkHashrate = 0; Coin_Base coin_base = new Coin_Base();
	 * 
	 * if(TosCoindApi.getInstance().can_connect().equals("true")){ dDifficulty =
	 * TosCoindApi.getInstance().getDifficulty(); dNetworkHashrate =
	 * TosCoindApi.getInstance().getNetworkHashps(); } else { dDifficulty = 1;
	 * dNetworkHashrate = 1; } return
	 * coin_base.calcExpectedNextDifficulty(dDifficulty,dNetworkHashrate); }
	 */
	...
	public double getBlocksUntilDiffChange() {
		...
		return Double.parseDouble(GlobalSettings.get("coindiffchangetarget"))
				- (iBlockcount % Double.parseDouble(GlobalSettings.get("coindiffchangetarget")));
	}
	...
	public Map<String, Object> getUserMiningStats(Connection conn, String username, int id, int interval) {
		...
		try {
			pstmt = conn.prepareStatement("" + "SELECT " + "IFNULL(SUM(difficulty) / ?, 0) AS sharerate, "
					+ "IFNULL(SUM(difficulty), 0) AS shares, " + "IFNULL(AVG(difficulty), 0) AS avgsharediff "
					+ "FROM (SELECT id, our_result, IF(difficulty = 0, POW(2, (" + GlobalSettings.get("difficulty")
					+ " - 16)), difficulty) AS difficulty " + "FROM shares "
					+ "WHERE username LIKE ? AND time > DATE_SUB(now(), INTERVAL ? SECOND) "
					+ "AND our_result = 'Y' UNION SELECT share_id, our_result, IF(difficulty = 0, POW(2, ("
					+ GlobalSettings.get("difficulty") + " - 16)), difficulty) AS difficulty " + "FROM shares_archive "
	...
	public Double getExpectedNextDifficulty() {
		Double dDifficultyValue = TosCoindApi.getInstance().getDifficulty();
		Double dDifficulty = dDifficultyValue == 0 ? 1 : dDifficultyValue;
		Double dNetworkHashrateValue = TosCoindApi.getInstance().getNetworkHashps();
		Double dNetworkHashrate = dNetworkHashrateValue == 0 ? 1 : dNetworkHashrateValue;
		Coin_Base coin_Base = new Coin_Base();
