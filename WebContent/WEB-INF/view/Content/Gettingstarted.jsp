 ...
 <div class="row">
    ...
    <p>1. <strong>Create account.</strong></p>
      <ul>
        <li>Register <a href="SignUp.do">here</a>, or login if you already have account</li>
        <li>Create a <a href="Account.do?action=workers">worker</a> that will be used by the miner to login</li>
      </ul>
    </li>
    <p>2. <strong>Download a miner.</strong></p>
      <ul>
        <c:if test="${algorithm == 'sha256' }">
    ...
    <ul>
      ...
      <li>CGMiner</li>
      <pre>./cgminer <c:if test="${algorithm == 'scrypt' }">--scrypt </c:if>-o stratum+tcp://${ SITESTRATUMURL}:${SITESTRATUMPORT} -u <em>Weblogin</em>.<em>WorkerName</em> -p <em>WorkerPassword</em></pre>
      <li>BFGMiner</li>
      <pre>./bfgminer <c:if test="${algorithm == 'scrypt' }">--scrypt </c:if>-o stratum+tcp://${SITESTRATUMURL}:${SITESTRATUMPORT} -u <em>Weblogin</em>.<em>WorkerName</em> -p <em>WorkerPassword</em></pre>
      <br />
      ...
      <pre>bfgminer <c:if test="${algorithm == 'scrypt' }">--scrypt </c:if>-o stratum+tcp://${SITESTRATUMURL}:${SITESTRATUMPORT} -u <em>Weblogin</em>.<em>WorkerName</em> -p <em>WorkerPassword</em></pre>
      <li>MinerD</li>
      <pre>minerd -a <c:if test="${algorithm == 'scrypt' }">scrypt </c:if>-t 6 -s 4 -o stratum+tcp://${SITESTRATUMURL}:${SITESTRATUMPORT} -u <em>Weblogin</em>.<em>WorkerName</em> -p <em>WorkerPassword</em></pre>
      <c:if test="${algorithm == 'scrypt' }">
      <li>Cudaminer For NVIDIA Cards</li>
	  <pre>cudaminer -o stratum+tcp://${SITESTRATUMURL}:${SITESTRATUMPORT} -u <em>Weblogin</em>.<em>WorkerName</em> -p <em>WorkerPassword</em></pre>
      </c:if>
	  <br />
	  <p> You then need to change "-u Weblogin.Worker -p Worker password" to reflect your own account. Eg, "-u Steve.StevesWorker -p StevesWorkerPassword" then go to "File > Save as" and save the file as "RunMe.bat" in the same folder containing your miners application files. You are now ready to mine, double click on "RunMe.bat" to start mining. If you want, you can create additional workers with usernames and passwords of your choice <a href="Account.do?action=workers">here</a></p>
    </ul>
    <p>4. <strong>Create a ${SITECOINNAME} address to receive payments.</strong></p>
      <ul>
        <li> Downloading the client &amp; block chain: 	Download the ${SITECOINNAME} client from <a href="${SITECOINURL}" target="_blank">here</a>.
